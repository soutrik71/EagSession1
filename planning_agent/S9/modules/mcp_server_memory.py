...
class MemoryStore:
    ...
    def _get_conversation_flow(self, conversation_id: str = None) -> Dict:
        ...
        return {
                    ...
                    "tool_calls": [
                        {
                            "tool": call["tool"],
                            "args": call["args"],
                            "result_summary": call.get(
                                "result_summary", "No summary available"
                            ),
                        }
            ...
            "timestamp_start": interactions[0].get("timestamp")
            if interactions
            else None,
            "timestamp_end": interactions[-1].get("timestamp")
            if interactions
            else None,
        }
...
async def get_current_conversations(input: Dict) -> Dict[str, Any]:
    ...
    try:
        ...
        memory_root = os.path.join(
            ROOT_DIR, "memory"
        )  # ROOT_DIR is already defined at top
        dt = datetime.now()

        # List all files in today's directory
        day_path = os.path.join(
            memory_root, str(dt.year), f"{dt.month:02d}", f"{dt.day:02d}"
        )
        ...
        return {
            "result": {
                ...
                "interactions": [
                    item for item in data if item.get("type") != "run_metadata"
                ],
            }
        }
    except Exception as e:
...
async def search_historical_conversations(input: SearchInput) -> Dict[str, Any]:
    ...
    try:
        ...
        for match in matches:
            match_text = " ".join(
                [str(match.get("user_query", "")), str(match.get("final_answer", ""))]
            )
