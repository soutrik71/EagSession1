...
sse_server_url = "http://127.0.0.1:4201/sse"

try:
    transport = SSETransport(url=sse_server_url)
    client = Client(transport)
except Exception as e:
    print(f"Error with SSETransport: {e}")
    raise e
...
async def main():
    """Test the SSE server with various mathematical operations, distance calculation, and weather."""

    print("📡 Testing FastMCP SSE Server")
    print(f"🔗 Connecting to: {sse_server_url}")
    print("=" * 60)
    ...
    try:
        ...
        print("\n📋 Troubleshooting:")
        print("   1. Check if the server is running on the correct port (4201)")
        print("   2. Verify the server URL is accessible")
        print("   3. Check firewall settings")
        print("   4. Ensure no other service is using port 4201")
        print("   5. Verify SSE endpoint is properly configured")
        print("=" * 60)
